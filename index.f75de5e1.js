...
function checkAvailableMoves() {
    ...
    for(let i = 0; i < cellsArray.length - width; i++){
        if (cellsArray[i].textContent === cellsArray[i + width].textContent && cellsArray[i].textContent !== "") return true;
    }
    for(let i = width; i < cellsArray.length; i++){
        if (cellsArray[i].textContent === cellsArray[i - width].textContent && cellsArray[i].textContent !== "") return true;
    }
    for(let i = 0; i < cellsArray.length; i++)if (i % width !== 0) {
        if (cellsArray[i].textContent === cellsArray[i - 1].textContent && cellsArray[i].textContent !== "") return true;
    }
    return false;
}
const width = 4;
function moveUp() {
    for(let i = 0; i < 4; i++){
        ...
        const column = [
            index.map((e)=>cellsArray[e].textContent)
        ];
...
function combineColumn() {
    for(let i = 0; i < 12; i++)if (cellsArray[i].textContent === cellsArray[i + width].textContent && cellsArray[i].textContent !== "") {
...
function combineRow() {
    for(let i = 0; i < 15; i++)if (cellsArray[i].textContent === cellsArray[i + 1].textContent && cellsArray[i].textContent !== "") {
        const combinedTotal = parseInt(cellsArray[i].textContent) * 2;
        cellsArray[i].textContent = combinedTotal;
...
function checkForWin() {
    for(let i = 0; i < cellsArray.length; i++)if (cellsArray[i].textContent == 2048) {
